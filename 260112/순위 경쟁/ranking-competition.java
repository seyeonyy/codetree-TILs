    ...
    public static void main(String[] args) {
        ...
        for(int i = 1; i <= n; i++){
            if(a[i-1] == b[i-1] && b[i-1] == c[i-1]){
                ...
                if(b[i] > c[i] && c[i] >= a[i] || b[i] > a[i] && a[i] >= c[i] || c[i] > a[i] && a[i] >= b[i] || c[i] > b[i] && b[i] >= a[i]){
                    cnt++;
                }
            }else if(b[i-1] > c[i-1] && c[i-1] >= a[i-1] || b[i-1] > a[i-1] && a[i-1] >= c[i-1]){
                if(a[i] > b[i] && b[i] >= c[i] || a[i] > c[i] && c[i] >= b[i] || c[i] > a[i] && a[i] >= b[i] || c[i] > b[i] && b[i] >= a[i]){
                    cnt++;
                }
            }else if(c[i-1] > a[i-1] && a[i-1] >= b[i-1] || c[i-1] > b[i-1] && b[i-1] >= a[i-1]){
                if(a[i] > b[i] && b[i] >= c[i] || a[i] > c[i] && c[i] >= b[i] || b[i] > a[i] && a[i] >= c[i] || b[i] > c[i] && c[i] >= a[i]){
                    cnt++;
                }
            }
        }
